...
class Ww4np2g5d(Network):
    ...
    def hidden_layers(self, network, **kwargs):
        ...
        network = Conv2DLayer(network, 128, (3, 3), pad=0,
            W=HeUniform('relu'))
        network = batch_norm(network, gamma=None)
        network = DropoutLayer(network, p=1.0/12)
        # 4th. Data size 50->48
        # EXPERIMENT
        # Add landmarks!
        # NO network = LandmarkLayer(network)
        ...
        network = batch_norm(network, gamma=None)
        network = DropoutLayer(network, p=1.0/6)
        # 6th. Data size 22->20
        # EXPERIMENT
        # Add landmarks!
        # NO network = LandmarkLayer(network)
        ...
        network = batch_norm(network, gamma=None)
        network = DropoutLayer(network, p=1.0/3)
